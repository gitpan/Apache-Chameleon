package Apache::Chameleon::Untaint::Path;
...
sub canonpath {
    ...
    #$path =~ s|(\./)+||gs unless $path eq "./";    # ./xx      -> xx
    $path =~ s|(\.+/)+||gs;
    $path =~ s|/(\.\./)+|/|gs;                     # /../../xx -> xx
...
Takes no argument, returns / as an array.
...
If $base is not present or '', then / is used. If $base is relative, 
...
This means that it is taken to be relative to /
